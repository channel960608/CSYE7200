...
object Function {
  ...
  /**
    ...
    */
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] = Try(f(t1y.get, t2y.get))
  ...
  /**
    ...
    * @return a value of R, wrapped in Try
    */
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = Try(f(t1y.get, t2y.get, t3y.get))

  /**
    * You get the idea...
    */
  def map7[T1, T2, T3, T4, T5, T6, T7, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3], t4y: Try[T4], t5y: Try[T5], t6y: Try[T6], t7y: Try[T7])
                                         (f: (T1, T2, T3, T4, T5, T6, T7) => R): Try[R] = Try(f(t1y.get, t2y.get, t3y.get, t4y.get, t5y.get, t6y.get, t7y.get))
  ...
  /**
    ...
    */
  // You know this one
  def lift[T, R](f: T => R): Try[T] => Try[R] = x => Try(f(x.get))
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = (t1, t2) => Try(f(t1.get, t2.get))
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = (t1, t2, t3) => Try(f(t1.get, t2.get, t3.get))
  ...
  (Try[T1], Try[T2], Try[T3], Try[T4], Try[T5], Try[T6], Try[T7]) => Try[R] = (t1, t2, t3, t4, t5, t6, t7) => Try(f(t1.get, t2.get, t3.get, t4.get, t5.get, t6.get, t7.get))
